...
export default function ReachartsTest() {
  const { isLoggedIn } = useContext(LocalSessionContext);
  ...
  function obtenerHoraParaDiaAnterior(
  ...
  const fechas: any = nuevoArreglo?.map(
    (item: { created_at: string | any[] }) => item.created_at.slice(0, 10),
  );

  const horas = nuevoArreglo?.map((item: { created_at: string | any[] }) =>
  ...
  */
  ...
  const medicionesFiltradas = meas?.filter(
...
*/
    ...
    const medicionesHorario2 = medicionesRevertidas.filter(
      (medicion: { created_at: string | number | Date }) => {
        const hora = new Date(medicion.created_at).getHours();
    ...
    const medicionesHorario3 = medicionesRevertidas.filter(
      (medicion: { created_at: string | number | Date }) => {
        const hora = new Date(medicion.created_at).getHours();
    ...
    const promedioHorario1 = {
      temperatura: calcularPromedio(
        medicionesHorario1.map(
          (medicion: { temperature: number }) => medicion.temperature,
        ),
      ),
      presionAtmosferica: calcularPromedio(
        medicionesHorario1.map(
          (medicion: { atm_pressure: number }) => medicion.atm_pressure,
        ),
      ),
      humedadRelativa: calcularPromedio(
        medicionesHorario1.map(
          (medicion: { rel_humidity: number }) => medicion.rel_humidity,
        ),
      ),
      velocidadViento: calcularPromedio(
        medicionesHorario1.map(
          (medicion: { wind_speed: number }) => medicion.wind_speed,
        ),
      ),
      humedadSuelo: calcularPromedio(
        medicionesHorario1.map(
          (medicion: { soil_moisture: number }) => medicion.soil_moisture,
        ),
      ),
    };
  ...
  return (
    <DashboardLayout>
      {false ? (
        ...
        <Fragment>
          <Title order={1}>
            Seleccionar día para mostrar variables microclimáticas
          </Title>
          ...
          <div style={{ margin: "10px 0 0 0" }}>
            <SimpleGrid cols={2}>
              ...
              >
                <label>
                  {" "}
                  Comportamiento de la Temperatura del día{" "}
              ...
              >
                <label>
                  Comportamiento de la Presión Atmosférica del día{" "}
                  {formattedDate.toString()}
                </label>
                <ResponsiveContainer width="100%" height="90%">
                  <BarChart
              ...
              >
                <label>
                  Comportamiento de la Humedad Relativa del día{" "}
                  {formattedDate.toString()}
                </label>
                <ResponsiveContainer width="100%" height="90%">
                  <BarChart
              ...
              >
                <label>
                  Comportamiento de la Velocidad del Viento del día{" "}
                  {formattedDate.toString()}
                </label>
                <ResponsiveContainer width="100%" height="90%">
                  <BarChart
              ...
              >
                <label>
                  Comportamiento de la Humedad del Suelo del día{" "}
                  {formattedDate.toString()}
                </label>
                <ResponsiveContainer width="100%" height="90%">
                  <BarChart
          ...
          */}
            ...
            <Modal opened={opened} onClose={close} withCloseButton={false}>
              <p>
                No hay datos disponibles para la fecha seleccionada. <br></br>
                Por favor, seleccione una fecha con datos.
              </p>
            </Modal>
          </div>
        </Fragment>
